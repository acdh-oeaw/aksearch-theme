...
function setupAutocomplete() {
  ...
  if (searchbox.hasClass("ac-auto-submit")) {
    acCallback = function autoSubmitAC(item, input) {
      // AK: When selecting an entry from the autocomplete drop-down at the search field,
      // add quotes around the search term if given condition(s) is/are met. At the moment,
      // this is done when a asterisk (*) is in the search term, becuase without quotes,
      // the result could be empty.
      // ORIGINAL (without quotes): input.val(item.value);
      if (item.value.includes('*')) {
        // AK: Add quotes to the search term
        input.val('"'+item.value+'"');
      } else {
        // AK: Search term without quotes
        input.val(item.value);
      }
